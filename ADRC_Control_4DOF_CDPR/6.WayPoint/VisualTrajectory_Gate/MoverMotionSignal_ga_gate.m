...
motion1Signal.ga = ...
  add_trace_ga_gate(t_windows(:,2),+1, motion1Signal.ga);
...
function ga = add_trace_ga_gate(t_window,dir,ga)
...
ga.signals.values = [ga.signals.values; dir .* ((0).*(0<=t&t<3)+((2.*pi).*s((t-3)./6)).*(3<=t&t<9)+((2.*pi)).*(9<=t&t<=12))];
end
